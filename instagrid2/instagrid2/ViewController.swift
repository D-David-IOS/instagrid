...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    // it's used in the function imagePickerController() for know the image will be changed
    var buttonSelect = -1
    ...
    // when the user presses a button, he can change the current image from his photo galery
    // if the image is already selected, he can change again the image
    ...
    // select1 and view1 are not hidden at launch
    ...
    // if the current orientation is portrait, the current view go to UP
    // if the current orientation is landscape, the current view go to Left
    ...
    @objc func share( selectedView : UIView) {
        ...
        act.completionWithItemsHandler = { (activityType, completed:Bool, returnedItems:[Any]?, error: Error?) in
            if completed {
